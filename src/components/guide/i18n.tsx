...
const en = {
  ...
  example: {
    ...
    lead: 'Three members, one week. Step through eight real tasks — or hit play — and watch the engine place each one: by skill, who frees up first, priority lanes (High/Urgent run in PARALLEL, stacked) and the Low cascade. Bar colour = priority.',
    members: [
    ...
    steps: [
      { task: 'Homepage redesign', meta: 'Graphic Design · Normal · 2 days', why: 'Everyone is free, so the engine assigns it — here Member A — blocking Mon and Tue.' },
      { task: 'Mobile app onboarding', meta: 'UX/UI · Normal · 2 days', why: 'Only Member B holds the UX/UI role, so affinity sends it straight to B.' },
      { task: 'Brand refresh', meta: 'Graphic Design · Normal · 1 day', why: 'A and B are busy; Member C is free now and frees up first — the date rule.' },
      { task: 'Sales one-pager', meta: 'Graphic Design · Normal · 1 day', why: 'C frees up Tuesday and takes the next Graphic Design task.' },
      { task: 'Launch teaser', meta: 'Graphic Design · High · 1 day', why: 'High has its OWN lane, so it doesn’t wait behind B’s Normal — it runs in PARALLEL, stacked on the same days.' },
      { task: 'Security hotfix', meta: 'Graphic Design · Urgent · 1 day', why: 'Same with Urgent: it slots in parallel on Member A, on top of the homepage work — Urgent never queues behind lower priorities.' },
      { task: 'Internal newsletter', meta: 'Graphic Design · Low · 1 day', why: 'A Low queues at the very end — Member C’s next free slot is Wednesday (grey bar).' },
      { task: 'Investor deck', meta: 'Graphic Design · Normal · 1 day', why: 'A Normal outranks a Low: it takes C’s Wednesday slot, and the newsletter Low — created today — is pushed to Thursday. “Push, don’t advance.”', highlight: true },
    ],
  },
  create: {
    title: 'Create a task',
...
const es: GuideContent = {
  ...
  example: {
    ...
    lead: 'Tres miembros, una semana. Avanza por ocho tareas reales —o dale a play— y mira cómo el motor coloca cada una: por especialidad, quién se libera primero, carriles de prioridad (High/Urgent corren en PARALELO, apilados) y la cascada de Low. Color de barra = prioridad.',
    members: [
    ...
    steps: [
      { task: 'Rediseño de homepage', meta: 'Graphic Design · Normal · 2 días', why: 'Todos están libres, así que el motor la asigna —aquí a Member A— bloqueando lun y mar.' },
      { task: 'Onboarding de la app móvil', meta: 'UX/UI · Normal · 2 días', why: 'Solo Member B tiene el cargo de UX/UI, así que la afinidad la manda directo a B.' },
      { task: 'Refresh de marca', meta: 'Graphic Design · Normal · 1 día', why: 'A y B están ocupados; Member C está libre ahora y se libera primero — la regla de la fecha.' },
      { task: 'One-pager de ventas', meta: 'Graphic Design · Normal · 1 día', why: 'C se libera el martes y toma la siguiente tarea de Graphic Design.' },
      { task: 'Teaser de lanzamiento', meta: 'Graphic Design · High · 1 día', why: 'Un High tiene su PROPIO carril, así que no espera tras el Normal de B — corre en PARALELO, apilado los mismos días.' },
      { task: 'Hotfix de seguridad', meta: 'Graphic Design · Urgent · 1 día', why: 'Igual con Urgent: entra en paralelo en Member A, encima del trabajo de homepage — un Urgent nunca hace cola tras prioridades menores.' },
      { task: 'Newsletter interno', meta: 'Graphic Design · Low · 1 día', why: 'Un Low hace cola al final del todo — el siguiente hueco de Member C es el miércoles (barra gris).' },
      { task: 'Deck para inversores', meta: 'Graphic Design · Normal · 1 día', why: 'Un Normal manda sobre un Low: toma el hueco del miércoles de C, y el Low del newsletter —creado hoy— se empuja al jueves. “Empujar, no adelantar.”', highlight: true },
    ],
  },
  create: {
