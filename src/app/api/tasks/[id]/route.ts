 ...
 */
export async function PUT(req: Request, { params }: TaskRouteParams) {
  try {
    const { id } = params; // El ID de la tarea es un String (ClickUp ID)
    ...
    await publishTaskUpdate({
      taskId: updatedTask.id,
      name: taskWithUpdatedAssignees?.name,
      status: taskWithUpdatedAssignees?.status,
      event: 'taskUpdated',
    });
 ...
 */
export async function DELETE(_req: Request, { params }: TaskRouteParams) {
  try {
    const { id } = params; // El ID de la tarea es un String (ClickUp ID)
    ...
    // Notificar a los clientes en tiempo real (Pusher)
    await publishTaskUpdate({ taskId: id, event: 'taskDeleted' });
